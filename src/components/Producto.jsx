...
export default function Producto({ producto, botonAgregar = false, botonDisponible = false }) {
    ...
    // Estado para manejar si el producto está disponible o agotado
    const [disponible, setDisponible] = useState(disponibleInicial);

    // Función para actualizar el estado de disponibilidad
    const toggleDisponible = async () => {
        await handleClickProductoAgotado(id); // Llamada a la API para actualizar el estado en el servidor
        setDisponible(prev => (prev === 1 ? 0 : 1)); // Cambiar el estado local del producto (1 -> 0 o 0 -> 1)
    };
